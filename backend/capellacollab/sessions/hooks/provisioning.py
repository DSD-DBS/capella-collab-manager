...
class ProvisionWorkspaceHook(interface.HookRegistration):
    ...
    async def async_configuration_hook(
        ...
        if cls._is_persistent_provisioning(
            request.provisioning, request.session_type
        ):
        ...
        if cls._is_persistent_provisioning(
            ...
            # TODO: Read/Create SessionProvisioning resource in database.
            # TODO: Skip provisioning if already there.

            resolved_entry = cls._resolve_provisioning_request(
                request.db, request.provisioning
            )[0]
    ...
    @classmethod
    def _is_persistent_provisioning(
        cls,
        ...
        return (
            len(provisioning) > 1
            and session_type == sessions_models.SessionType.PERSISTENT
        )
