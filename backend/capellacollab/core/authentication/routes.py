...
async def api_get_token(
    ...
    tokens = flow.exchange_code_for_tokens(
        token_request.code, token_request.code_verifier
    )

    user = validate_id_token(db, tokens["id_token"], token_request.nonce)
...
async def api_refresh_token(
    ...
    user = validate_id_token(db, tokens["id_token"], None)
    update_token_cookies(
        response, tokens["id_token"], tokens.get("refresh_token", None), user
    )
...
def validate_id_token(
    db: orm.Session, id_token: str, nonce: str | None
    ...
    if config.authentication.client.id not in validated_id_token["aud"]:
