...
class ProjectUserScopes(core_pydantic.BaseModel):
    ...
    t4c_model_links: set[t.Literal[permissions_models.UserTokenVerb.GET]] = (
        pydantic.Field(
            default_factory=set,
    ...
    provisioning: set[t.Literal[permissions_models.UserTokenVerb.GET]] = (
        pydantic.Field(
            default_factory=set,
            title="Provisioning",
            description=(
    ...
    restrictions: set[
        ...
        description=(
            "Manage restrictions on models"
            " (Provide access to pure::variants)"
        ),
    )
    ...
    def __and__(self: t.Self, other: "ProjectUserScopes"):
        ...
        for attribute, verbs in other:
            derived_scope[attribute] = derived_scope[attribute] & verbs
