...
def upgrade():
    ...
    with op.batch_alter_table('models') as batch_op:
        ...
        models = connection.execute(sa.select(
            t_models.c.id,
            t_models.c.name,
            t_projects.c.id,
        ).select_from(joined))
        ...
        for id_, name, project_id in models:
            ...
            connection.execute(t_models.update().where(t_models.c.id == id_).values(
                slug=slug,
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('models') as batch_op:
        batch_op.add_column(sa.Column('project_name', sa.String(), autoincrement=False, nullable=True))
    ...
    connection = op.get_bind()
    connection.execute(t_models.update().where(
        t_models.c.project_id == t_projects.c.id
    ).values(
        project_name=t_projects.c.name
    ))

    with op.batch_alter_table('models') as batch_op:
        batch_op.create_foreign_key('capella_models_project_name_fkey', 'projects', ['project_name'], ['name'])
